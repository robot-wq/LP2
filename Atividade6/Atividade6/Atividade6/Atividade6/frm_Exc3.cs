        ...
        private void btn_Executar_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < 2; i++)
            {
                for (int j = 0; j < 10; j++)
                {
                    ...
                    if (string.IsNullOrEmpty(respostas[i,j]))
                    {
                        return;
                    }
            ...
            for (int k = 0; k < 2; k++) {
                for (int d = 0; d < 10; d++) { 
                    if (gabarito[d] == respostas[k, d])
                    lst_Verificador.Items.Add($"Aluno {k+1}  acertou era {gabarito[d]}, escolheu{respostas[k, d]}");
                    else
                        lst_Verificador.Items.Add($"Aluno {k + 1}  errou era {gabarito[d]}, escolheu{respostas[k, d]}");
                }
            }
        }
    }
